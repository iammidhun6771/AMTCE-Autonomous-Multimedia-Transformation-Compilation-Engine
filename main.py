...
# Ensure the actual package root is on sys.path before the repo root.
# In this workspace, the importable AMTCE package tree lives under
# `simpler update/`, so that path must win the resolver order.
_REPO_ROOT = os.path.dirname(os.path.abspath(__file__))
_SAMPLE_UPDATE_ROOT = os.path.join(_REPO_ROOT, "simpler update")

# Remove any direct sample-tree path entries that would otherwise shadow the
# real package roots. We then place the canonical package root first and keep
# the workspace root available only as a fallback.
...
if _SAMPLE_UPDATE_ROOT not in sys.path:
    sys.path.insert(0, _SAMPLE_UPDATE_ROOT)
            ...
            if bot_token and target_chat:
                try:
                    ...
                    async def _send_single():
                        ...
                        payload_path = reel_path
                        real_cid = os.path.basename(payload_path).replace("_master.mp4", "")
                        possible_raw = os.path.join(os.path.dirname(os.path.abspath(__file__)), "downloads", real_cid, "video.mp4")

                        # ── PHASE 2.1 WATERMARK GATE (interrupt before Telegram send) ──
                        watermark_output = os.path.splitext(payload_path)[0] + "_wm_cleaned.mp4"
                        try:
                            from Watermark_and_Inpainting.watermark_main import run_watermark_removal
                            logger.info(f"🧼 [PHASE 2.1] Running watermark cleanup on rendered master before Telegram delivery: {os.path.basename(payload_path)}")
                            cleaned_video_path, _watermark_log = run_watermark_removal(
                            ...
                            if cleaned_video_path and os.path.exists(cleaned_video_path):
                                payload_path = cleaned_video_path
                                logger.info(f"✅ [PHASE 2.1] Watermark gate completed. Telegram payload now uses: {os.path.basename(payload_path)}")
                        ...
                        with open(payload_path, "rb") as vf:
                        ...
                        if storage_group and str(target_chat) != str(storage_group):
                            try:
                                with open(payload_path, "rb") as svf:
