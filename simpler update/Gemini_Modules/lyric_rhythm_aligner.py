...
def analyze_music(audio_path: str) -> Dict[str, Any]:
    ...
    # 💾 CACHE HIT GUARD: If persistent lyric intelligence exists, return immediately without API call
    if os.path.exists(cache_json_path) and os.path.getsize(cache_json_path) > 50:
        try:
            with open(cache_json_path, "r", encoding="utf-8") as f:
                cached_report = json.load(f)
            if isinstance(cached_report, dict) and cached_report.get("has_vocals") is not None:
    ...
    # ⚡ FAST PATH: Build instant rhythm report using BeatEngine if API upload is slow
    try:
        ...
        # Save to persistent cache so subsequent runs hit disk cache in 0.000s
        try:
            ...
            try:
                ...
                AudioPoolManager().merge_lyric_into_pool(
                    os.path.basename(audio_path), fast_report
                )
            except Exception as _pm:
                logger.debug(f"[LYRIC_ALIGNER] pool merge (fast_path): {_pm}")
        except Exception:
            pass
        return fast_report
    except Exception as _fe:
        logger.warning(f"Fastpath beat engine fallback error: {_fe}")
        return _empty_report()
    ...
    # Wrap entire upload + generation in a 10.0s hard timeout worker thread
    analysis_res = []
    ...
    if not analysis_res or isinstance(analysis_res[0], Exception):
        logger.warning(f"[LYRIC_ALIGNER] 10s Budget Exceeded / Error for '{os.path.basename(audio_path)}' — proceeding with fallback.")
        print(f"  └─ ⚠️ [LYRIC_ALIGNER] 10s budget reached — proceeding with fast fallback.", flush=True)
        return _empty_report()
